...
## old <- read.GeneMapper("cssr1234.txt") #reads the file
...
## tmpTrip <- triploids
## PopNames(tmpTrip) <- levels(factor(c("South", "North")))
## PopInfo(tmpTrip) <- factor(popTable[Samples(tmpTrip), "area"])
...
## This doesn't work as expected - we lose all the loci data?
## diploidAG <- gendata.to.genind(genambig.to.genbinary(diploids))
## diploidAGpop <- genind2genpop(diploidAG)
## diploidAGStrata <- data.frame(species = pop(diploidAG),
##                             population = popTable[rownames(diploidAG@tab),"population" ])
## strata(diploidAG) <- diploidAGStrata
## The result works with poppr for determining MLG values though!
...
## That seems rather round about, but I                                        ##
## couldn't find a direct genambig-genind conversion.                          ##
#################################################################################
